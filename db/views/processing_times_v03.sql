with base as (
  SELECT
    ...
    CASE
      WHEN (application_version.application ->> 'import_date')::timestamp IS NOT NULL THEN true
      ELSE false
    END AS claim_imported
  from application_version join application on application_version.application_id = application.id and application_version.pending is false
)
