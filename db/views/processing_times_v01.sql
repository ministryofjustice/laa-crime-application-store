with base as (
  SELECT
    ...
    -- get the status from the last_updated_at for the given application id
    -- if no previous version assume it's a draft
    COALESCE(LAG(application_version.application ->> 'last_updated_at', 1) OVER (
        PARTITION BY application_version.application_id
        ORDER BY version
      ), application_version.application ->> 'created_at')::timestamp from_time,
    application_version.application ->> 'status' as to_status,
    (application_version.application ->> 'last_updated_at')::timestamp as to_time
