SELECT
  ...
  payable_claims.original_submission_date AS original_submission_date,
  payable_claims.youth_court AS youth_court,
  payable_claims.client_last_name AS client_last_name,
  ...
  CASE
    WHEN (
      payment_requests.request_type IN ('non_standard_mag_supplemental', 'non_standard_mag_appeal', 'non_standard_mag_amendment')
      AND payment_requests.calculation_method = 'entered_to_be_paid'
      AND payment_requests.payment_basis IN ('linked_no_original_payment', 'new_unlinked_record')
    ) THEN 'Y'
    ELSE 'N'
  END AS contingency,
  CASE
