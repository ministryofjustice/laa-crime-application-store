module PaymentRequests
  ...
  private
    ...
    def nsm_claim_details
      {
        ...
        original_submission_date: Date.new(params[:original_submission_year].to_i, params[:original_submission_month].to_i, 1),
        laa_reference: params[:linked_laa_reference] || generate_laa_reference,
        ufn: params[:ufn],
        idempotency_token: params[:idempotency_token],
        nsm_claim_type: params[:nsm_claim_type],
      }
    end
    ...
    def persist_linked_submission!(claim)
      ...
      return unless linked_submission&.id == submission_id

      claim.update!(submission_id: linked_submission.id)
    end
  end
end
