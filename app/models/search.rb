class Search < ApplicationRecord
  ...
  def self.where_terms(string)
    ...
    sub_strings = string.strip.downcase.split(/\s+/).map do |str|
      if str.start_with?("laa-")
        ...
        escape_and_assign_category(clean_str, "A")
      elsif /\A\d+\/\d+\z/.match?(str) then escape_and_assign_category(str, "A")
      elsif /\A\d+{6}\z/.match?(str) then escape_and_assign_category(str, "*")
      else
        escape_and_assign_category(str.tr("/", "-"), "*B")
      end
    end
  ...
  def self.escape_and_assign_category(string, category)
    %('''#{string.gsub("'", "''")}''':#{category})
  end
end
