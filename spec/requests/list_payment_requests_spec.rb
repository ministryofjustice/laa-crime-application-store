...
RSpec.describe "list payment request", type: :request do
  ...
  context "with payment requests" do
    ...
    it "returns expected keys" do
      ...
      get "/v1/payment_requests"
      expect(response.parsed_body.first.keys.sort).to eq(payment_request_keys.sort)
    end
    ...
    it "returns expected payment_request_claim keys" do
      ...
      get "/v1/payment_requests"
      expect(response.parsed_body.first["payment_request_claim"].keys.sort).to eq(payable_keys.sort)
    end
  end
end
