...
RSpec.describe "Link submission payment search services" do
  ...
  describe BaseSearchService do
    let(:client_role) { :caseworker }
    let(:dummy_search_service) do
      stub_const(
        "DummySearchService",
        Class.new(BaseSearchService) do
          def search_query
            []
          end
    ...
    describe "#call" do
      it "calculates pagination offsets" do
        result = dummy_search_service.new({ page: 2, per_page: 5 }, client_role).call

        expect(result).to eq(offset: 5, limit: 5, per_page: 5, page: 2)
      end

      it "defaults pagination values" do
        result = dummy_search_service.new({}, client_role).call
  ...
  describe Crm7SubmissionClaim do
